...
class Allocate():
    def __init__(self, dis_C2BS, dis_D, dis_C2D, dis_D2C, dis_BS2D, dis_DiDj, dis_D2BS, numD2DReciver):
        ...
        self.dataUEMax = config["dataUEMax"]                        #UE的最大資料量
        self.dataUEMin = config["dataUEMin"]                        #UE的最小資料量
        ...
        self.g_b, self.g_d = gain.cell_downlink()
        self.g_c, self.g_d = gain.cell_uplink()
    ...
    def alloc_downlink(self, time, c_x, c_y):
        ...
        ueData = np.random.randint(low=self.dataUEMin, high=self.dataUEMax, size=len(scheduleUE))   #BS生成要傳輸給UE的資料量
        ...
        for index in candicateUE:
            for rb in range(self.numRB):
                if scheduleUE[index] == 0:
                    snr = self.tool.calculate_SNR(self.firPb, self.g_b[index][rb], self.N0)
                    snr_db = self.convert.mW_to_dB(snr)
                    ueSNR[index][rb] = snr
                    ueSNR_db_rb[index][rb] = snr_db

                elif scheduleUE[index] == 1 or scheduleUE[index] == 2:
                    snr = self.tool.calculate_SNR(self.secPb, self.g_b[index][rb], self.N0)
                    snr_db = self.convert.mW_to_dB(snr)
                    ueSNR[index][rb] = snr
                    ueSNR_db_rb[index][rb] = snr_db
        ...
        #beamCandicate, ueMinSINR_dB, assignRB, rbStatus, ueSNR_db (可能會用到的回傳值,依序分別為：波束編號,UE最小SINR,UE使用哪些RB,RB分配狀況,UE的SNR)
    ...
    def alloc_uplink(self, c_x, c_y, directCUE, omnidirectCUE):
        ...
        ueData = np.random.randint(low=self.dataUEMin, high=self.dataUEMax, size=len(candicateUE))                              #候選UE生成資料量
        ...
        for i in range(len(candicateUE)):
            ...
            for rb in range(self.numRB):
                power = self.convert.SNR_to_Power(upperSinr, self.g_c[candicateUE[i]][rb], self.N0)
        ...
        #candicateUE, bsMinSINR_dB, bsSNR_db, powerUE, assignRB, rbStatus
