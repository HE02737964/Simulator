...
class Genrator():
    ...
    def generateGroupTxPoint(self, numUE, radius, cluster, numDensity):
        nr = np.sqrt(np.random.rand(1, cluster)) * self.radius
        alpha = np.random.rand(1, cluster) * 2 * np.pi
        rx = nr * np.cos(alpha)
        ry = nr * np.sin(alpha)
        randomPoint = np.vstack([rx,ry]).T

        numDensityUE = int(numUE * (numDensity / 100))
        numGeneratUE = numUE - numDensityUE
        total = numDensityUE
        point = []
        for group in range(cluster):
            ue = np.array([(numDensityUE / cluster)])
            ue = np.ceil(ue)
            numGroupUE = int(ue[0])
            if total < numGroupUE:
                numGroupUE = total
            nr = np.sqrt(np.random.rand(1, numGroupUE)) * radius
            alpha = np.random.rand(1, numGroupUE) * 2 * np.pi
            x = randomPoint[group][0] + nr * np.cos(alpha)
            y = randomPoint[group][1] + nr * np.sin(alpha)

            location = (x**2 + y**2).flatten()
            index_out = np.where(location > self.radius**2)[0]
            len1 = len(index_out)
            x = np.delete(x, index_out)
            y = np.delete(y, index_out)

            while len1:
                nr = np.sqrt(np.random.rand(1, len1)) * radius
                alpha = np.random.rand(1, len1) * 2 * np.pi
                xt = nr * np.cos(alpha)
                yt = nr * np.sin(alpha)
            ...
            tx_point = np.vstack([x,y]).T
            for i in range(len(tx_point)):
                point.append(tx_point[i])

        nr = np.sqrt(np.random.rand(1, numGeneratUE)) * self.radius
        alpha = np.random.rand(1, numGeneratUE) * 2 * np.pi
        gx = nr * np.cos(alpha)
        gy = nr * np.sin(alpha)
        tx_point = np.vstack([gx,gy]).T
        for i in range(len(tx_point)):
            point.append(tx_point[i])
