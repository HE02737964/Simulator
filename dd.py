...
for d2d in range(numD2D):
    ...
    i = (background * g_d2d[3][d2d][0][0]) + (powerList_mW[2] * g_d2d[2][d2d][0][0])
...
tbs, rb = tool.data_tbs_mapping(data[2], 1)
cqi = convert.TBS_CQI_mapping(tbs)
sinr = convert.CQI_SINR_mapping(cqi)
minSinr = convert.dB_to_mW(sinr)
print('When D2D',2,'use d2d 1 RB')
print('minSINR',sinr)

ib = background * g_d2d[3][2][0][0] + (powerList_mW[0] * g_d2d[0][2][0][0])
SINR = (powerList_mW[2] * g_d[d2d][0][0]) / (N0 + ib)
SINR_DB = convert.mW_to_dB(SINR)
print('sinr',SINR_DB)

i3 = (powerList_mW[1] * g_d2d[1][2][0][0]) + (background * g_d2d[3][2][0][0])
iSINR = (powerList_mW[2] * g_d[2][0][0]) / (N0 + i3)
iSINR_DB = convert.mW_to_dB(iSINR)
# print('power {} x gain {} / N0 {} + i {} = sinr {}'.format(powerList_mW[d2d], g_d[d2d][0][0], N0, i3, (powerList_mW[d2d] * g_d[d2d][0][0]) / (N0 + i3)))
print('When D2D',2,'use d2d 2 RB')
print('minSINR',iSINR_DB)
print()
